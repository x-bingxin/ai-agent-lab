...
from llm_client import EmbeddingModelEntry, get_llm_client
import json

class RAGPipeline:
    """完整的 RAG 回答通道"""

    def __init__(self, embed_service: EmbeddingService, doc_store: DocumentStore, llm_client):
        self.embed = embed_service
        self.store = doc_store
        self.llm = llm_client

    async def ingest(self, text: str, source: str = 'unknown'):
        """摄入文档，分块，向量化，存储"""
        chunks = chunk_document(text, {'source': source})
        chunk_texts = [c['content'] for c in chunks]
        embeddings = await self.embed.embed(chunk_texts)

        metadatas = [
            {'source': source, 'chunk_index': c['index'], 'content': c['content']} for c in chunks
        ]

        ids = self.store.add_documents(chunk_texts, embeddings, metadatas)
    ...
    async def stream_query(self, question: str, top_k: int = 3):
        ...
        for doc, meta, dist in zip(
            ...
            context_parts.append(f"[来源： {meta.get('source', 'unknown') if meta else ''} {doc}]")

        context = "\n\n ---- \n\n".join(context_parts)
        print(f"\n\n问题 {question} 找出的上下文包括：{context}")
...
async def main():
    ...
    doc = """这是一个神话故事，传说在很久很久以前，天下分为东胜神洲、西牛贺洲、南赡部洲、北俱芦洲。在东胜神洲傲来国，有一座花果山，山上有一块仙石，一天仙石崩裂，从石头中滚出一个卵，这个卵一见风就变成一个石猴，猴眼射出一道道金光，向四方朝拜。
